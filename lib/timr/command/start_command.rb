	...
	module Timr
		...
		class StartCommand < Command
			...
			def initialize(argv = [])
				...
				# optparser.register_option(['-n', '--name'], 1)
				# optparser.register_option(['-d', '--description'], 1)
				# optparser.register_option(['--date'], 1)
				# optparser.register_option(['-t', '--time'], 1)
				# optparser.register_option(['-m', '--message'], 1)
				# opts = optparser.parse(argv)
				
				# optparser.verify(argv)
				# opts = argv
